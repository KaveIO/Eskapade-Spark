...
writers['df_generic_writer'] = SparkDfWriter(name='df_generic_writer', read_key='df', write_methods=['csv'], num_files=num_files)

# create generic RDD-writer link
writers['rdd_generic_writer'] = SparkDfWriter(name='rdd_generic_writer', read_key='rdd', schema=columns, write_methods=['csv'], num_files=num_files)

# create RDD-CSV-writer link
writers['rdd_csv_writer'] = SparkDataToCsv(name='rdd_csv_writer', read_key='rdd', output_path='{}/rdd_csv'.format(output_dir), mode='overwrite', sep=separator, header=columns if write_header else False, num_files=num_files)
