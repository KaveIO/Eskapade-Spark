...
import eskapadespark as package

# Fixtures
_FIXTURE = {_.name: _ for _ in pathlib.Path(resource_filename(package.__name__, 'data')).glob('*')}

# C/C++ libraries that are shipped with eskapade.
if sys.platform == 'darwin':
    _LIBS = {_.name: _ for _ in pathlib.Path(resource_filename(package.__name__, 'lib')).glob('*.dylib')}
else:
    _LIBS = {_.name: _ for _ in pathlib.Path(resource_filename(package.__name__, 'lib')).glob('*.so')}

# Tutorials that are shipped with eskapade.
_TUTORIALS = {_.name if _.parent.name == 'tutorials' else _.parent.name + '/' + _.name:
              _ for _ in pathlib.Path(resource_filename(package.__name__, 'tutorials')).glob('**/*.py')}

# Templates that are shipped with eskapade.
_TEMPLATES = {_.name: _ for _ in pathlib.Path(resource_filename(package.__name__, 'templates')).glob('*')}

# Configs that are shipped with eskapade.
_CONFIGS = {_.name: _ for _ in pathlib.Path(resource_filename(package.__name__, 'config')).glob('**/*')}
...
def _resource(resource_type, name: str) -> str:
    ...
    raise FileNotFoundError('Could not find {resource_type} "{name!s}" at {apath}! Does it exist?'
                            .format(resource_type=resource_type, name=name, apath=full_path))
